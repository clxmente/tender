...
export default function Likes({ LikedRecipes, LikedRestaurants }) {
	...
	// no likes
	if (LikedRecipes.length == 0 && LikedRestaurants.length == 0) {
		return (
			<div>
...
Likes.getInitialProps = async (ctx) => {
	...
	var restaurants = JSON.parse(window.sessionStorage.getItem("restaurants"));
	...
	return { LikedRecipes: recipes, LikedRestaurants: restaurants };
};
